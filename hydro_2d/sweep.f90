...
contains
  ...
  subroutine sweep(u_prim, du_con_dt, x_in, x_if_in, nx, dir)
    ...
    real,     intent(inout) :: du_con_dt(1:nx,5)
    ...
    u  (1:nx) = u_prim(1:nx,iu)
    ut (1:nx) = u_prim(1:nx,iut)
    ...
    do i = 1, nx
      du_con_dt(i,1) = du_con_dt(i,1) + 1.0 / dx(i) * (rhoflx (i-1) - rhoflx (i))
      du_con_dt(i,iu) = du_con_dt(i,iu) + 1.0 / dx(i) * (momflx (i-1) - momflx (i))
      du_con_dt(i,iut) = du_con_dt(i,iut) + 1.0 / dx(i) * (momtflx (i-1) - momtflx (i))
      du_con_dt(i,4) = du_con_dt(i,4) + 1.0 / dx(i) * (momttflx (i-1) - momttflx (i))
      du_con_dt(i,5) = du_con_dt(i,5) + 1.0 / dx(i) * (eneflx (i-1) - eneflx (i))
    enddo
